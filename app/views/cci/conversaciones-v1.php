				...
				<div class="cci-reply-box">
					...
					<div id="responder-tab" class="cci-reply-content active">
						<?php if ($isFreshchatConversation && !$freshchatReplyWindowOpen): ?>
							<div class="alert alert-warning mb-2" style="font-size: 0.88rem;">
								<i class="bi bi-clock-history"></i>
								La ventana de 24 horas de WhatsApp está cerrada<?= $freshchatLastInboundAt !== '' ? ' desde el último mensaje recibido el ' . e($freshchatLastInboundAt) : '' ?>.
								Envía una plantilla aprobada desde Freshchat o espera que el contacto escriba nuevamente.
							</div>
						<?php endif; ?>
						<form method="POST" action="<?= e(base_url('cci/conversaciones/' . $selectedId . '/reply')) ?>" enctype="multipart/form-data" class="cci-reply-form">
							<?= csrf_field() ?>
							<textarea class="cci-reply-textarea" name="reply_text" rows="3" maxlength="10000" placeholder="<?= $isFreshchatConversation && !$freshchatReplyWindowOpen ? 'Ventana de respuesta cerrada' : 'Escribe tu mensaje aquí...' ?>" id="cci-reply-text" <?= $isFreshchatConversation && !$freshchatReplyWindowOpen ? 'disabled' : '' ?>></textarea>
							...
							<div style="display: flex; gap: 8px; align-items: center; width: 100%; flex-wrap: wrap;">
								...
								<button class="btn btn-primary btn-sm" type="submit" title="Enviar mensaje" <?= $isFreshchatConversation && !$freshchatReplyWindowOpen ? 'disabled' : '' ?>>
